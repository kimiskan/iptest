...
$('#commentForm').submit(function () {
  if (ValidateIPaddress(document.querySelector('#testip'))) {
    $('.res').removeClass('res');
    $.ajax({
      type: "POST",
      url: urlDir + '/sendSOAP.php',
      data: { ip: $('#testip').val() },
      cashe: false,
      beforeSend: function () {//действие при начале отправки
        $('#result').text('Минуточку...');
      },
